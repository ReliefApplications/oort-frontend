...
/** Question type */
const QUESTION_TYPE = 'users';

/**
 * Question users model.
 */
export class QuestionUsersModel extends Question {
  /**
...
/**
 * Register serialization properties so SurveyJS knows how to read/write this question type from/to JSON
 */
Serializer.addClass(
  QUESTION_TYPE,
  [],
  () => new QuestionUsersModel(''),
  'question' // inherit from base question
);

/**
 * Register in the element factory so type users works in JSON
 */
ElementFactory.Instance.registerElement(QUESTION_TYPE, (name: string) => {
  return new QuestionUsersModel(name);
});
